...
> Documento de entrega. El detalle de proceso, las decisiones intermedias y los
> resultados fase por fase están en el documento de trabajo
> ([informe.md](informe.md)); los notebooks 01-06 contienen todas las tablas y
...
servicio, escaneo de puertos, fuerza bruta de contraseñas, ataques web, botnet,
infiltración, Heartbleed).
...
- **Sin fuga de información:** el escalado se ajusta dentro del pipeline (solo
  ...
  submuestreo) ocurre dentro del pipeline de `imbalanced-learn`, nunca sobre
...
Se cruzaron dos ejes en una matriz de 2 × 4 (capacidad del modelo × técnica de
desbalance), con validación cruzada idéntica:
...
1. **La capacidad del modelo pesó más que la técnica de desbalance.** El peor
   ...
   Web Attack, con AP de 0,21 y 0,20 en la línea base lineal) no estaban
...
herramientas de ataque configuran la apertura de la conexión TCP distinto que
...
Un Isolation Forest se entrenó **únicamente con los 394.236 flujos benignos del
lunes** (el único día sin ataques) del conjunto de entrenamiento, con umbral
fijado en el cuantil 1% de sus propios scores. Evaluado sobre el resto de la
...
- **Los que no ve — el punto ciego del enfoque:** fuerza bruta (FTP/SSH),
  PortScan, ataques web y Bot tienen recall ≈ 0. **Cada flujo individual de
...
estructuralmente raros y nunca etiquetados, aporta valor real. El costo
operativo adicional es la **deriva temporal**: el tráfico benigno del viernes
en la tarde dispara 9,6% de falsas alarmas (frente a ~1% los demás días).
...
P(Bot) ≥ 0,999 (elegido con probabilidades out-of-fold del entrenamiento, para
...
| Con puerto, sin regla de umbral (argmax) | 0,967 | 0,964 |
| **Estimación conservadora: sin el puerto (argmax)** | **0,942** | **0,937** |
